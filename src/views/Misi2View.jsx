...
import { misi2DetailData } from '../data/mockData';
import {
  ResponsiveContainer,
  LineChart,
  Line,
...
const sasaranColors = {
  s1: { bg: 'from-emerald-600 to-teal-600', border: 'border-emerald-500/30', badge: 'bg-emerald-500/20 text-emerald-300', light: 'bg-emerald-50 dark:bg-emerald-950/30', accent: '#10b981' },
  s2: { bg: 'from-amber-600 to-orange-600', border: 'border-amber-500/30', badge: 'bg-amber-500/20 text-amber-300', light: 'bg-amber-50 dark:bg-amber-950/30', accent: '#f59e0b' },
};
...
export default function Misi2View({ darkMode, onNavigate }) {
  const data = misi2DetailData;
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-1 text-[11px] text-slate-500 dark:text-slate-400 font-semibold">
          ...
          <ChevronRight className="w-3 h-3" />
          <span className="text-emerald-600 dark:text-emerald-400">Misi 2</span>
        </div>
      </div>

      {/* ===== TUJUAN PEMBANGUNAN HERO ===== */}
      <div className="rounded-2xl bg-gradient-to-r from-emerald-600 via-teal-600 to-emerald-700 p-5 text-white relative overflow-hidden shadow-xl">
        ...
        <div className="relative z-10">
          ...
          <div className="mt-3 flex items-center gap-3 text-[10px] flex-wrap">
            ...
            <span className="flex items-center gap-1 bg-white/15 px-2 py-1 rounded-full">
              <Wheat className="w-3 h-3" /> Hilirisasi Pertanian & Ekonomi Kerakyatan
            </span>
          </div>
        </div>
      </div>

      {/* ===== INDIKATOR UTAMA (PDRB, Daya Saing, LPE) ===== */}
      <div>
        <h3 className="text-sm font-extrabold text-slate-900 dark:text-white mb-3 flex items-center gap-2">
          ...
          <span className="text-[9px] font-bold text-slate-400">(Tujuan Misi 2)</span>
        </h3>
              ...
              >
                ...
                <div className="grid grid-cols-3 gap-2 mb-3">
                  ...
                  <div className="bg-slate-100 dark:bg-slate-800/60 rounded-lg p-2 text-center">
                    ...
                    <p className="text-sm font-extrabold text-emerald-600 dark:text-emerald-400">{ind.target[5].nilai}</p>
                  </div>
                </div>
                ...
                <div className="h-28">
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={ind.target}>
                      <defs>
                        <linearGradient id={`grad-m2-utama-${idx}`} x1="0" y1="0" x2="0" y2="1">
                      ...
                      <Area type="monotone" dataKey="nilai" stroke={ind.color} strokeWidth={2.5} fill={`url(#grad-m2-utama-${idx})`} dot={{ r: 3.5, fill: ind.color, stroke: '#fff', strokeWidth: 2 }} activeDot={{ r: 5 }} name="Target RPJMD" />
                    </AreaChart>
                  </ResponsiveContainer>
                </div>
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2 text-[9px] mb-2">
                          ...
                          <div className="flex-1 text-center p-1.5 rounded-lg bg-slate-50 dark:bg-slate-800/60">
                            ...
                            <p className="font-extrabold text-emerald-600 dark:text-emerald-400">{ind.target[5].nilai}</p>
                          </div>
                        </div>
                        ...
                        <div className="h-20">
                          <ResponsiveContainer width="100%" height="100%">
                            <LineChart data={ind.target}>
                              ...
                              <Tooltip content={<CustomChartTooltip />} />
                              {ind.realisasi > 0 && (
                                <ReferenceLine y={ind.realisasi} stroke="#f59e0b" strokeDasharray="4 4" strokeWidth={1} />
                              )}
                          ...
                          <div className="mt-3 pt-3 border-t border-slate-200 dark:border-slate-700">
                            <div className="h-44">
                              <ResponsiveContainer width="100%" height="100%">
                                <AreaChart data={ind.target}>
                                  <defs>
                                    <linearGradient id={`grad-m2-${sasaran.id}-${idx}`} x1="0" y1="0" x2="0" y2="1">
                                      <stop offset="5%" stopColor={ind.color} stopOpacity={0.3} />
                                      <stop offset="95%" stopColor={ind.color} stopOpacity={0.02} />
                                    </linearGradient>
                                  </defs>
                                  ...
                                  <Area type="monotone" dataKey="nilai" stroke={ind.color} strokeWidth={2.5} fill={`url(#grad-m2-${sasaran.id}-${idx})`} dot={{ r: 4, fill: ind.color, stroke: '#fff', strokeWidth: 2 }} name="Target RPJMD" />
                                </AreaChart>
                              </ResponsiveContainer>
                            </div>
      ...
      <div className="rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden">
        <div className="bg-slate-900 dark:bg-slate-950 px-4 py-3 flex items-center justify-between">
          <h3 className="text-xs font-extrabold text-white flex items-center gap-2">
            ...
            Tabel Target RPJMD 2025–2030 — Misi 2
          </h3>
          ...
          <table className="w-full text-[10px]">
            <thead>
              <tr className="bg-slate-100 dark:bg-slate-800/80 text-slate-600 dark:text-slate-300 text-left">
                ...
                <th className="px-3 py-2 font-bold sticky left-8 bg-slate-100 dark:bg-slate-800/80 min-w-[220px]">Indikator</th>
                <th className="px-3 py-2 font-bold text-center">Satuan</th>
                <th className="px-3 py-2 font-bold text-center">Realisasi</th>
                <th className="px-3 py-2 font-bold text-center bg-emerald-50 dark:bg-emerald-950/30">2025</th>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60">
              ...
              <tr className="bg-emerald-50/50 dark:bg-emerald-950/20">
                <td colSpan={11} className="px-3 py-1.5 text-[9px] font-black text-emerald-700 dark:text-emerald-300 uppercase tracking-wider">
                    ...
                    {ind.target.map((t, tidx) => (
                      <td
                        ...
                        className={`px-3 py-2 text-center font-bold ${
                          tidx === 0 ? 'bg-emerald-50/50 dark:bg-emerald-950/20 text-emerald-700 dark:text-emerald-300' :
